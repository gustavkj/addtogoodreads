...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  isbnScan = request.content.match(
    /ISBN(-*1(?:(0)|3))?:?\s*?(97(8|9))?\d{9}(\d|X)/i
  );
  isbnScan2 = request.content.match(
    /ISBN(-*1(?:(0)|3))?:?\s[0-9]{1,}(\s|-)[0-9]{1,}(\s|-)[0-9]{4,}(\s|-)[0-9]{1,}(\s|-)(([0-9]|x){1,})*/i
  );
  ...
  if (isbnScan != null) {
    ...
    isbnResult = tmp.match(/([0-9|x]{10,13})/gim);
    console.log("result" + isbnResult);
    console.log("result type " + typeof isbnResult);
  } else if (isbnScan2 != null) {
    var tmp = isbnScan2[0];
    isbnResult = tmp.match(
      /[0-9]{1,}(\s|-)[0-9]{1,}(\s|-)[0-9]{4,}(\s|-)[0-9]{1,}(\s|-)(([0-9]|x){1,})*/gm
    );
    console.log("result" + isbnResult);
  }
  ...
  if (isbnResult != null) {
    ...
    req.open(
      "GET",
      "http://www.goodreads.com/book/isbn?isbn=" +
        isbn +
        "&" +
        "key=PkY9lbrcAVS2dHn1DidESg",
      true
    );
    ...
    function printDesc() {
      if (req.status == 200) {
        var descriptions = req.responseXML.getElementsByTagName(
          "reviews_widget"
        );
        ...
        for (var i = 0, desc; (desc = descriptions[i]); i++) {
          ...
          goodreadsContent =
            '<iframe height="110" width="325" frameborder="0" scrolling="no" src="';
          goodreadsContent +=
            "http://www.goodreads.com/book/add_to_books_widget/" +
            isbn +
          ...
          goodreadsContent +=
            "<i>" + title[0].textContent + "</i> by: " + author[0].textContent;
